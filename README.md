...
```python
from agentsmith_sdk import AgentSmithLogger

logger = AgentSmithLogger(log_dir=".tmp/agent_logs")
logger.set_context(conversation_id="conv-1", request_id="req-1")

# 记录事件（自动维护 span_id/parent_span_id 树形结构）
logger.log("REQUEST_START", name="用户请求", input_data={"query": "hello"})
logger.log("LLM_START", name="GPT调用", input_data={"prompt": "..."})
logger.log("LLM_END", name="GPT调用", output_data={"response": "..."})
logger.log("REQUEST_END", name="用户请求", output_data={"result": "done"})
```
...
files = scanner.scan(".tmp/agent_logs")
for f in files:
    events = parser.parse(f)
    pusher.push(events, server_url="http://localhost:18900")
```
